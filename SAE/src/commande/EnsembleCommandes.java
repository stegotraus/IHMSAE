...
public class EnsembleCommandes
{
    ...
    /**
     * Méthode testant si l'ensembledes de commandes est vide.
     * @return true si et seulement si l'ensemble de commande est vide
     */
    ...
    {
        for (int x = 0; x < commandes.size(); x++)
        {
            if (commandes.get(x).getReference() == refCommande)
            {
                commandes.remove(x);
                return true;
            }
    ...
    {
        Commande[] totalCommandes = new Commande[commandes.size()];
        for (int x = 0; x < commandes.size(); x++)
        {
            totalCommandes[x] = commandes.get(x);
        }
    ...
    public void afficher()
    {
        for (int x = 0; x < commandes.size(); x++)
        {
            System.out.println(commandes.get(x).toString());
        }
    }
